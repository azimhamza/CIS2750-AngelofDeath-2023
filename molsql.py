...
#---------------------------------------------------------------
    ...
    def radial_gradients(self):
        ...
        ## Function that amount of atoms and bonds from that molecule 
    def count_atoms_bonds(name):
        c = db.conn.cursor()
        c.execute("SELECT COUNT(*) FROM MoleculeAtom INNER JOIN Molecules ON MoleculeAtom.MOLECULE_ID=Molecules.MOLECULE_ID WHERE Molecules.MOLECULE_NAME=?", (name,))
        atom_count = c.fetchone()[0]
        c.execute("SELECT COUNT(*) FROM MoleculeBond INNER JOIN Molecules ON MoleculeBond.MOLECULE_ID=Molecules.MOLECULE_ID WHERE Molecules.MOLECULE_NAME=?", (name,))
        bond_count = c.fetchone()[0]
        c.close()
        return atom_count, bond_count
    
    # Function that displays all the elements in the database
    def get_all_elements(self):
        c = self.conn.cursor()
        c.execute("SELECT * FROM Elements")
        elements = c.fetchall()
        c.close()
        elements_list = [{'element_no': el[0], 'element_code': el[1], 'element_name': el[2],
                        'colour1': el[3], 'colour2': el[4], 'colour3': el[5], 'radius': el[6]}
                        for el in elements]
        return elements_list
    
    # Function that displays all the molecules in the database
    def get_all_molecules(self):
        c = self.conn.cursor()
        c.execute("SELECT * FROM Molecules")
        molecules = c.fetchall()
        c.close()
        molecules_list = [{'molecule_id': mol[0], 'name': mol[1]} for mol in molecules]
        return molecules_list
    
    def add_element(self, element_no, element_code, element_name, colour1, colour2, colour3, radius):
        c = self.conn.cursor()
        c.execute('''INSERT OR IGNORE INTO Elements (ELEMENT_NO, ELEMENT_CODE, ELEMENT_NAME, COLOUR1, COLOUR2, COLOUR3, RADIUS)
                        VALUES (?, ?, ?, ?, ?, ?, ?)''', (element_no, element_code, element_name, colour1, colour2, colour3, radius))
        self.conn.commit()

    def remove_element(self, element_code):
        c = self.conn.cursor()

        # Delete all related atoms in the Atoms table
        c.execute("DELETE FROM Atoms WHERE ELEMENT_CODE=?", (element_code,))

        # Delete the element from the Elements table
